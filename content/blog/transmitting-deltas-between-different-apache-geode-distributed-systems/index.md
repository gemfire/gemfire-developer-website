...
---
## Introduction
VMware GemFire provides a [Delta](https://docs.vmware.com/en/VMware-GemFire/10.0/gf/developing-delta_propagation-chapter_overview.html) interface that facilitates serializing the changes to an object between two JVMs rather than the entire object when changes are made to that object. For large objects, this provides an optimization that is supported from:
...
- The [CacheWriter](https://docs.vmware.com/en/VMware-GemFire/9.15/gf/developing-events-list_of_event_handlers_and_events.html) on the data Region in the server sets the [GatewaySender](https://docs.vmware.com/en/VMware-GemFire/10.0/gf/topologies_and_comm-multi_site_configuration-setting_up_a_multisite_system.html) queue key
- The [CacheListener](https://docs.vmware.com/en/VMware-GemFire/10.0/gf/developing-events-list_of_event_handlers_and_events.html) on the data Region in the server creates an [EntryEvent](https://docs.vmware.com/en/VMware-GemFire/10.0/gf/developing-events-list_of_event_handlers_and_events.html) on the proxy Region containing the appropriate bytes (either delta or full) and distributes that event to the GatewaySender attached to the proxy Region
- The GatewaySender attached to the proxy Region sends the event to a server in the receiving DistributedSystem
- A [GatewayReceiver](https://docs.vmware.com/en/VMware-GemFire/10.0/gf/topologies_and_comm-multi_site_configuration-setting_up_a_multisite_system.html) in a server on the receiving DistributedSystem receives the event
...
The implementation consists of the following three [CacheCallback](https://docs.vmware.com/en/VMware-GemFire/10.0/gf/developing-events-list_of_event_handlers_and_events.html) classes:
...
- invokes the `LocalRegion` `basicBridgePut()` or `basicBridgeDestroy()` method depending on the `EntryEvent`’s operation and boolean callback argument. The basicBridgePut method is invoked with either the full bytes or delta bytes from the input `EntryEvent`.
